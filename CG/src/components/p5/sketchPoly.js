import {graphJeu} from "./sketchJeu";
...
class Vertex{
        ...
        static connectVertex(_v1 , _v2  ){
            // eslint-disable-next-line @typescript-eslint/no-use-before-define
            const  e1 = new Edges(  _v1 ,_v2 , couleur ) ;
            // eslint-disable-next-line @typescript-eslint/no-use-before-define
            const e2 = new Edges( _v2 , _v1  , couleur) ;
...
export let graphPoly  ;
export let actualisePoly = false ;
export let pointMemoPoly = null ;
export let couleur = 'blue' ;
...
export function draw(p5 ) {
        ...
        if(actualisePoly) {
            ...
            p5.strokeWeight(4);
            graphPoly.draw(p5);
            console.log( "graph durant le draw :", graphPoly ) ;
            p5.stroke('purple');
            if(pointMemoPoly != null ){
                pointMemoPoly.draw(p5) ;
            }
...
export function mousePressed(p5Poly) {
    //console.log("souris :" ,p5Poly.mouseX ,p5Poly.mouseY) ;
        if(p5Poly.mouseX < 800 && p5Poly.mouseY< 800   &&  p5Poly.mouseX > 0 && p5Poly.mouseY> 0 ) { //on verrifie que l'in est bien dans la fenetre
            ...
            if (pointMemoPoly === null) { // on as aucun point
                ...
                const droite = new DroiteGeo(pointMemoPoly, p, graphPoly);
...
export function getGraph(){
    ...
    const polygone = graphPoly.listPolygone ;
    const edges = graphPoly.listEdge ;
...
export let graphePoly = null ;
export function buildGraph(p5 ){
    graphePoly = new Graph() ;
    console.log( graphePoly ) ;
    graphePoly.draw( p5 , true) ;
    //console.log( graph.getSmallPath() ) ;
}

export function GetSmallPath( p5 ) {
    const path =  graphePoly.getSmallPath(p5) ;
    console.log(path) ;
    p5.background(200);
    graphPoly.draw(p5) ;
    graphPoly.draw(p5 , false) ;
    path.draw(p5) ;
}
