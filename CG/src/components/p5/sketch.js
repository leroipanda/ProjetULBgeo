...
class Vertex{
        ...
        static connectVertex(_v1 , _v2  ){
            const  e1 = new Edges(  _v1 ,_v2   ) ;
            const e2 = new Edges( _v2 , _v1 ) ;
...
class DCEL{
        ...
        insertInEdge( vertex1 , vertex2 , x, y ){
            ...
            this.connectVertex( vertex1 , vertex ) ;
            this.connectVertex( vertex2 , vertex ) ;
            return vertex
...
export let graph  ;
export let actualise = false ;
export let pointMemo = null ;
...
export function mousePressed(p5) {
        if(p5.mouseX < 800 && p5.mouseY< 800 ) { //on verrifie que l'in est bien dans la fenetre
            //console.log(graph) ;
            console.log("pls click on the graph to put a line");
            const p = new PointGeo(p5.mouseX, p5.mouseY);
            if (pointMemo === null) { // on as aucun point
                ...
                const droite = new DroiteGeo(pointMemo, p, graph);
...
export function getGraph(){
    ...
    const polygone = graph.listPolygone ;
    const edges = graph.listEdge ;
...
class Path {
    constructor(_noeuds , _noeusPasse ) {
        ...
        getNextPath(){
            ...
            this.noeudsActu.listEdge.forEach( e => {
                e.listNoeuds.forEach( n => {
                    ...
                    if(  !dejaVu(n , this.noeusPasse)){
                        //console.log("new path");
                        nextPath.push( new Path (n , [...this.noeusPasse] ))
                    }
...
export function GetSmallPath( p5 ) {
    const path =  graphe.getSmallPath(p5) ;
    console.log(path) ;
    p5.background(200);
    graph.draw(p5) ;
    graphe.draw(p5 , false) ;
    path.draw(p5) ;
}
