...
class TriangleGeo {
        ...
        ptInside(pt ){
            ...
            /*console.log(this.pt1 , this.pt2 ,this.pt3 , pt  );
...
class PolyGeo {
        ...
        //function used to know if a point is inside a polygon
        isInside( x ,y ){
            let trouve =false ;
            this.ears.forEach( triangle =>{

               const t = triangle.ptInside( new PointGeo( x,y ));
        ...
        getNodeObject(){
            ...
            //we compute a center of the polygon
            let x= 0 ;
            let y = 0 ;
            this.listeVertex.forEach(
                e => {
...
class Vertex{
        ...
        static connectVertex(_v1 , _v2  ){
            // eslint-disable-next-line @typescript-eslint/no-use-before-define
            const  e1 = new Edges(  _v1 ,_v2 , couleurJeu ) ;
            // eslint-disable-next-line @typescript-eslint/no-use-before-define
            const e2 = new Edges( _v2 , _v1  , couleurJeu) ;
...
export let graphJeu  ;
export let actualiseJeu = false ;
export let pointMemoJeu = null ;
export let couleurJeu = 'blue' ;
export let pathJeu = [] ;

let pathJeuDraw =  null;
let lastColor = null ;
...
export function draw(p5 ) {
        ...
        if(actualiseJeu) {
            ...
            p5.background(200);
            p5.strokeWeight(4);
            //console.log( "graph avant le draw :", graphJeu ) ;

            if(pathJeuDraw !== null){
                //console.log("smmoth path ", pathJeuDraw) ;
            ...
            graphJeu.draw(p5);
            //console.log( "graph apres le draw :", graphJeu ) ;
            //on Dessine le chemin


            p5.stroke('purple');
            if(pointMemoJeu != null ){
                pointMemoJeu.draw(p5) ;
            }
...
export function mousePressed(p5Poly) {
    function includeCustom (liste , obj) {
    ...
    //console.log("souris :" ,p5Poly.mouseX ,p5Poly.mouseY) ;
        if(p5Poly.mouseX < 800 && p5Poly.mouseY< 800   &&  p5Poly.mouseX > 0 && p5Poly.mouseY> 0 ) { //on verrifie que l'in est bien dans la fenetre
            //on regarde all the polygone to know in which one our point is
            //console.log("on recommence..") ;
            let polygoneTrouve = null ;
            graphJeu.listPolygone.forEach( p =>{
                    if( p.isInside(p5Poly.mouseX , p5Poly.mouseY)){
                        //console.log("=======poly trouve =========:",p.id);
                        polygoneTrouve = p ;
                    }
                });
            //si on ne l'a pas visité on j'ajoute au path
            if(includeCustom( pathJeu ,   polygoneTrouve )){
                //console.log("deja visite");
                //on fait rien
            }
            else {
                actualiseJeu = true;
                //on doit verifier que le polygone est bien un polygone voisin
                // eslint-disable-next-line @typescript-eslint/no-use-before-define
                const dualGraph = new Graph();
                const noeudsDual = dualGraph.nodes[polygoneTrouve.id] ;
                let trouve = false ;

                let colorEdges = "" ;
                if(pathJeu.length !== 0 ) {

                    noeudsDual.listEdge.forEach(edges => {
                        edges.listNoeuds.forEach(noeudsVoisin => {
                            //console.log(noeudsVoisin.name, pathJeu[pathJeu.length - 1].id);
                            if (parseInt(noeudsVoisin.name) === pathJeu[pathJeu.length - 1].id) {
                                trouve = true;
                                //console.log("trouve");
                                colorEdges = edges.color ;
                            }
                ...
                if(  (trouve && colorEdges!==lastColor )  || pathJeu.length ===0 ){
                    //console.log("ajoute") ;
                    pathJeu.push(polygoneTrouve) ;
                    lastColor = colorEdges;

                    if(pathJeu.length ===2) {

                        // eslint-disable-next-line @typescript-eslint/no-use-before-define
                        pathJeuDraw = new Path("", [], []);
                        pathJeuDraw.noeusPasse= [];
                        pathJeu.forEach(poly => {
                            pathJeuDraw.noeusPasse.push(dualGraph.nodes[poly.id]);

                        });
                        pathJeuDraw.colorPasse.push(colorEdges) ;
                    ...
                    if(pathJeu.length >2){
                        ...
                        pathJeuDraw.colorPasse.push(colorEdges) ;
                        pathJeuDraw.noeusPasse.push(dualGraph.nodes[polygoneTrouve.id]);
                    }
...
export function getGraph(){
    ...
    const polygone = graphJeu.listPolygone ;
    const edges = graphJeu.listEdge ;
...
export function getJson(){
    return graphJeu.toJson();
}

export function importJson(objJson){
    const obj =  JSON.parse( JSON.stringify( objJson ) );
    console.log("import" ,obj , obj.vertex);
    const vertexHashmap = {};
    const newDCEl = new DCEL();
    //clean
    newDCEl.listPolygone = [];
    newDCEl.listEdge = [] ;
    newDCEl.listVertex = [];
    newDCEl.borderVertex = [] ;
    newDCEl.border = [ ];
    obj.vertex.forEach( v =>{

            console.log(v);
            const ver = newDCEl.addVertex(v.x, v.y);
            vertexHashmap[v.id] = ver;
    ...
    obj.edges.forEach( e => {
        ...
        if(paire) {
            ...
            const edges = newDCEl.connectVertex(vertexHashmap[e.start], vertexHashmap[e.end]);
            edges[0].color = e.color ;
            edges[1].color = e.color ;
            paire =false ;
        }
        else {
            paire = true;
    ...
    /*obj.border.forEach( bd => {
        newDCEl.borderVertex.push(  vertexHashmap[bd.id] ) ;
    });*/
    for( let i = 0 ; i < obj.border.length ; i++){
        newDCEl.borderVertex.push( vertexHashmap[obj.border[i].id]) ;
    }


    obj.borderEdges.forEach( b =>{
        newDCEl.border.push( vertexHashmap[b.start].getEdge(vertexHashmap[b.start] ,vertexHashmap[b.end]) );
        //console.log( vertexHashmap[b.start].getEdge(vertexHashmap[b.start] ,vertexHashmap[b.end]) )
    });

    //console.log (newDCEl , vertexHashmap);
    newDCEl.getBordure() ;
    //console.log (newDCEl , vertexHashmap);
    graphJeu = newDCEl ;
...
export function getPathLongeur(){
    if (  pathJeu.length === 0 ){
       return 0
    }
    else{
        return pathJeu.length -1   ;
    }

}
export function previous() {
    pathJeu.pop();
    pathJeuDraw.colorPasse.pop() ;
    pathJeuDraw.noeusPasse.pop() ;
    actualiseJeu = true ;
    lastColor = pathJeuDraw.colorPasse[ pathJeuDraw.colorPasse.length -1] ;
...
export let grapheJeu = null ;
export function buildGraph(p5 ){
    grapheJeu = new Graph() ;
    //console.log( grapheJeu ) ;
    grapheJeu.draw( p5 , true) ;
    //console.log( graph.getSmallPath() ) ;
}

export function GetSmallPath( p5 ) {
    const path =  grapheJeu.getSmallPath(p5) ;
    //console.log(path) ;
    p5.background(200);
    graphJeu.draw(p5) ;
    graphJeu.draw(p5 , false) ;
    path.draw(p5) ;
}
